...
👉 Here we’re saying: “When the event named greet happens, run this function.” The callback (name) => { ... } will get any data passed with the event. In this case, it prints a personalized greeting with the provided name. js // Emit the "greet" event eventEmitter.emit("greet", "Ayush");
...
👉 Selects the collection users where documents will be stored. js const newUser = { name: "Ayush Tomar", email: "ayushtomar@.com", age: 21, createdAt: new Date() }; 
